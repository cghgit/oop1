...
public class newdish {
    ...
    String dishNames[] = new String[4];//餐品名称
    String addresses[] = new String[4];//地址
    int times[] =new int[4];//送餐时间
    int states[] = new int[4];//订餐状态
    double sumprice[] =new double[4];//总计金额
    ...
    public void checkOrder(){
        ...
        for (int i = 0; i < 4; i++) {
            if (names[i]!=null){
                ...
                String price1 = sumprice[i]+"元";
                String statu = states[i]==0?"已完成":"已预订";
                System.out.println((i+1)+"\t"+"\t"+names[i]+"\t"+"\t"+dishNames[i]+"\t"+time1+"\t"+addresses[i]+"\t"+"\t"+price1+"\t"+statu);
            }
        }
        int choice;
        do{
